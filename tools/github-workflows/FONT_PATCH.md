# Необязательный патч: свой шрифт для темы «Виноград» (Comfortaa)

**Статус: ЗАМЕНЁН общим пакетом шрифтов тем — смотрите FONTS_THEME_PATCH.md
(та же Comfortaa для «Винограда» + Manrope/Russo One/PT Serif для остальных
тем, единая инструкция загрузки). Этот файл оставлен как историческая памятка.**
...
```
/* 🍇 Виноград — «маркетплейс»-характер (0.3.19,
```
...
После сборки: Настройки → Оформление → 🍇 Виноград — весь текст должен стать
