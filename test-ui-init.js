...
// sport-шрифт только в заголовках, компактные сегменты настроек.
{
  ...
  const okVer = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
...

{
  ...
  const okVer050 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7')
...

{
  ...
  const okVer051 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7') && fs.existsSync('tools/make-pro-code.js');
  if (!okVer051) failed++;
...

{
  ...
  const okVer052 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer052) failed++;
...

{
  ...
  const okVer053 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer053) failed++;
...

{
  ...
  const okVer054 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer054) failed++;
...

{
  ...
  const okVer055 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer055) failed++;
...

{
  ...
  const okDesc065 = descR.includes('Актуально для версии:') && descR.includes('0.7.7');
  if (!okDesc065) failed++;
  console.log(`${okDesc065 ? '✓' : '✗'} 0.5.6 APP_DESCRIPTION.md: единое описание приложения на месте и актуально`);

  const okVer065 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer065) failed++;
...

{
  ...
  const okVer057 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer057) failed++;
...

{
  ...
  const okVer058 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer058) failed++;
...

{
  ...
  const okVer060 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer060) failed++;
...

{
  ...
  const okVer070 = appR.includes("const FITFLOW_VERSION = '0.7.7'") && html.includes('v0.7.7');
  if (!okVer070) failed++;
